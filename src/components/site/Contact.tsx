...
export const Contact = () => {
  ...
  return (
    <section id="contact" className="relative py-12 sm:py-20 md:py-32 overflow-hidden bg-secondary/30">
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[min(100%,800px)] max-w-[100vw] h-[min(50vw,400px)] min-h-[200px] bg-primary-glow/5 rounded-full blur-3xl pointer-events-none" />
      <div className="container relative grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12">
        <div className="animate-fade-in-up min-w-0">
          ...
          <h2 className="font-display text-2xl sm:text-3xl md:text-5xl font-bold mt-2 mb-4 sm:mb-6 leading-tight text-balance">
            Share your <span className="text-gradient">requirement</span>
          </h2>
          <p className="text-muted-foreground text-base sm:text-lg mb-6 sm:mb-10 max-w-prose">
          ...
          <div className="mt-6 sm:mt-8 p-4 sm:p-6 bg-card border border-border rounded-2xl relative overflow-hidden group">
        ...
        <form onSubmit={onSubmit} className="bg-card border border-border rounded-2xl p-4 sm:p-6 md:p-8 shadow-elegant space-y-4 min-w-0 animate-slide-in-right">
...
const Info = ({ icon, title, value, href }: { icon: React.ReactNode; title: string; value: string; href?: string }) => {
  ...
  return (
    <Wrap
      href={href}
      className={`flex items-start gap-3 sm:gap-4 p-3 sm:p-4 bg-card border border-border rounded-xl hover:border-primary-glow/50 hover:-translate-y-0.5 transition-all duration-300 group min-w-0${href ? " cursor-pointer" : ""}`}
    >
      <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-xl bg-primary/5 text-primary grid place-items-center shrink-0 group-hover:gradient-accent group-hover:text-primary-foreground transition-all duration-500">
        {icon}
      </div>
      <div className="min-w-0">
